...
if [[ -z "$MOXYGEN_DIR" ]]; then
  ...
  # A configured build dir has already fetched this exact rev — CPM records
  # where it put it, and the build binds to MOXYGEN_REV, so the recorded tree is
  # the pinned one. Reuse it rather than keeping a second full copy per pin.
