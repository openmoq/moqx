...
case "${1:-}" in
  setup|configure) die "configuring is done by scripts/configure.sh [PROFILE] --mode (prebuilt|from-source)" ;;
...
[[ -f "$build_dir/CMakeCache.txt" ]] \
  || die "no configured build dir '$build_dir' — run first: scripts/configure.sh $profile --mode (prebuilt | from-source)"
