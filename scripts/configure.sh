...
#                                  source build. Start here — moxygen decides what
#                                  it publishes, so this is the only value that
#                                  always yields a build. The fallback compiles the
#                                  whole folly stack.
#   --moxygen prebuilt:            download the prebuilt install for the pinned
#                                  rev (fast); no published tarball for this
#                                  rev/platform is an error. Take it when a failure
#                                  suits you better than a long build.
...
# variables; override with the MOQX_MOXYGEN_PROFILE env var). Neither prebuilt nor
# prebuilt-with-fallback can deliver one, so both refuse a sanitizer preset unless
# MOQX_ALLOW_UNINSTRUMENTED_DEPS=1 — and the fallback then builds the uninstrumented
# stack too, so losing the prebuilt cannot silently turn a short lane into a long
# instrumented one.
#
...
# Two ways to lose the instrumented moxygen a sanitizer build needs: a --moxygen
# value that cannot produce one, and MOQX_MOXYGEN_PROFILE naming a different one.
# Neither is visible to the moqx build, so refuse both here. The mismatch case is
# not only a downgrade: an ASan moxygen under a TSan moqx links two sanitizer
# runtimes that cannot coexist.
if [[ "$needs_profile" != default && -z "${MOQX_ALLOW_UNINSTRUMENTED_DEPS:-}" \
      && ( "$moxygen" != from-source || "$moxygen_profile" != "$needs_profile" ) ]]; then
  die "preset '$profile' enables sanitizers ($needs_profile) and needs a moxygen instrumented to match
  (currently: $moxygen_profile) —
...
# cache when it is, so the configure that follows needs no network. The probe's
# own knobs must be forwarded, or it would answer for a different platform or
# release than the moqx configure goes on to ask for.
fetch_prebuilt() {
...
case "$resolved" in
  ...
  prebuilt-with-fallback)
    # Ask the fetcher directly rather than reading a failed moqx configure as
    # "no prebuilt": a broken CMakeLists or an unrelated CPM fetch would
    # otherwise buy a folly build that fails at the same place half an hour
    # later. On success the install is already in the cache, so the configure
    # below reuses it without touching the network.
