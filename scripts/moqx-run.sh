...
#
# This script ends in exec (and --dry-run prints the path for inspection), so it
# cannot delete its own file on the way out. Instead each run sweeps the ones left
# by earlier runs: moqx reads the config once at startup and has no reload path,
# so a file this old is not open by anything. The private 0700 directory is what
# makes the sweep safe to point at a glob.
RUN_TMP="${TMPDIR:-/tmp}/moqx-run-$(id -u)"
if [[ -L "$RUN_TMP" ]]; then
  echo "error: $RUN_TMP is a symlink; refusing to write resolved configs there" >&2
  exit 1
fi
mkdir -p "$RUN_TMP"
if [[ ! -O "$RUN_TMP" ]]; then
  echo "error: $RUN_TMP is not owned by $(id -un)" >&2
  exit 1
fi
chmod 700 "$RUN_TMP"
find "$RUN_TMP" -maxdepth 1 -type f -name 'moqx-resolved.*' -mtime +0 -delete 2>/dev/null || true

RESOLVED_CONFIG="${MOQX_RESOLVED_CONFIG:-$(mktemp "$RUN_TMP/moqx-resolved.XXXXXX")}"
