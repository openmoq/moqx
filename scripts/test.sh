...
[[ -f "$build_dir/CMakeCache.txt" ]] || { echo "test.sh: no configured build dir '$build_dir' — build first (scripts/configure.sh $profile --moxygen …, scripts/build.sh $profile)" >&2; exit 1; }
