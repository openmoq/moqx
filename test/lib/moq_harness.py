...
class Harness:
    ...
    def wait_relay_ready(self, name):
        ...
        for _ in _until(READY_TIMEOUT):
            ...
            self._dump_relay_log(name)
            raise HarnessError(
                f"relay {name} not ready after {READY_TIMEOUT:g}s "
                f"({self._admin_error})"
            )
        if relay.proc is not None and relay.proc.poll() is not None:
            raise HarnessError(
    ...
    def wait_sessions_atleast(self, name, want):
        ...
        self._dump_relay_log(name)
        raise HarnessError(
            f"relay {name}: moqActiveSessions={value} < {want} "
            f"after {READY_TIMEOUT:g}s"
        )
    ...
    def relay_stop(self, name):
        ...
        if status == 0:
            ...
            self._fail(
                f"[{name}] relay exited {status} on SIGTERM — crash or "
                "sanitizer leak"
            )
            self._dump_relay_log(name)
    ...
    def expect_settled(self, settle, *names):
        ...
        if not names:
            names = [
                n for n, r in self.relays.items() if not (r.killed or r.stopped)
            ]
        if not names:
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="moqx relay topology integration test"
    )
    parser.add_argument(
