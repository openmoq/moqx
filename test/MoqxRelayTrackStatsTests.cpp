...
TEST_P(MoQRelayTrackStatsTest, CountsIngestObjectsAndBytes) {
  ...
  EXPECT_EQ(ingest.groupsReceived, 1);
  EXPECT_EQ(ingest.subgroupsReceived, 1);
  EXPECT_EQ(ingest.objectsReceived, 2);
  EXPECT_EQ(ingest.bytesReceived, 30);
...
TEST_P(MoQRelayTrackStatsTest, CountsGroupsByTransition) {
  ...
  auto ingest = trackCounters(kTestTrackName);
  EXPECT_EQ(ingest.groupsReceived, 3);
  EXPECT_EQ(ingest.subgroupsReceived, 3);
  EXPECT_EQ(ingest.objectsReceived, 3);
  EXPECT_EQ(ingest.bytesReceived, 15);
...
TEST_P(MoQRelayTrackStatsTest, CountsEgressPerSubscriber) {
  ...
  EXPECT_EQ(egress.objectsSent, 2);
  EXPECT_EQ(egress.bytesSent, 20);
  EXPECT_EQ(egress.subgroupsSent, 2);
  EXPECT_EQ(egress.groupsSent, 2);
...
TEST_P(MoQRelayTrackStatsTest, CountsEgressOnPublishFanout) {
  ...
  EXPECT_EQ(counters.objectsReceived, 1);
  EXPECT_EQ(counters.objectsSent, 1);
  EXPECT_EQ(counters.bytesSent, 7);
...
TEST_P(MoQRelayTrackStatsTest, CountsInterleavedGroupsOnce) {
  ...
  auto counters = trackCounters(kTestTrackName);
  EXPECT_EQ(counters.groupsReceived, 2);
  EXPECT_EQ(counters.subgroupsReceived, 4);
...
TEST_P(MoQRelayTrackStatsTest, ActiveGroupSurvivesWindowChurn) {
  ...
  EXPECT_EQ(trackCounters(kTestTrackName).groupsReceived, window + 1);
...
TEST_P(MoQRelayTrackStatsTest, RecountsGroupEvictedFromWindow) {
  ...
  EXPECT_EQ(trackCounters(kTestTrackName).groupsReceived, window + 2);
