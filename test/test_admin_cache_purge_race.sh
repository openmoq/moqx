...
# 3. All Phase 1 storm responses had skipped >= 1 (live track was encountered,
#    not just that no error occurred) and contained no error field.
...
# Every response must have skipped >= 1 (the track was live throughout).
SKIPPED_ZERO=$(grep -c '"skipped":0' "$PURGE_LOG" 2>/dev/null || true)
if [[ "$SKIPPED_ZERO" -gt 0 ]]; then
  echo "FAIL [3]: $SKIPPED_ZERO storm response(s) had skipped:0 — live track should have blocked eviction" >&2
  grep '"skipped":0' "$PURGE_LOG" >&2; exit 1
fi
echo "PASS [3]: all $STORM_COUNT storm responses had skipped>=1, no errors (live track correctly blocked)"

# 4. Phase 2 purge evicted the track now that sessions are gone.
EVICTED=$(echo "$PHASE2" | grep -o '"evicted":[0-9]*' | cut -d: -f2 || true)
SKIPPED=$(echo "$PHASE2" | grep -o '"skipped":[0-9]*' | cut -d: -f2 || true)
if [[ -z "$EVICTED" || "$EVICTED" -lt 1 ]]; then
  echo "FAIL [4]: Phase 2 purge did not evict anything (got: $PHASE2) — eviction path not exercised" >&2; exit 1
fi
if [[ -n "$SKIPPED" && "$SKIPPED" -gt 0 ]]; then
  echo "FAIL [4]: Phase 2 purge had skipped=$SKIPPED — expected 0 after all sessions drained" >&2; exit 1
fi
echo "PASS [4]: Phase 2 evicted=$EVICTED skipped=$SKIPPED — eviction path confirmed"
