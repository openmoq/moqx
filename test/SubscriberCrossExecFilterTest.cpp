...
// without the safe point it resumes on a destroyed filter and reads targetExec_
// to build the handle. See PublisherCrossExecFilterTest for the same shape.
  ...
  EXPECT_CALL(*inner_, publishNamespace(_, _))
      .WillOnce(
          [nsHandle,
           &evb,
           &innerEntered,
           &innerRelease](PublishNamespace, std::shared_ptr<Subscriber::PublishNamespaceCallback>)
              -> folly::coro::Task<Subscriber::PublishNamespaceResult> {
