...
TEST(ResolveConfig, MinimalInsecure) {
  ...
  EXPECT_EQ(resolved.services[0].name, "default");
  EXPECT_EQ(resolved.services[0].cache.maxCachedTracks, 100u);
  EXPECT_EQ(resolved.services[0].cache.maxCachedGroupsPerTrack, 3u);
}
...
TEST(ResolveConfig, CacheDisabled) {
  ...
  ASSERT_TRUE(result.hasValue());
  const auto& resolved = result.value().config;
  EXPECT_EQ(resolved.services[0].cache.maxCachedTracks, 0u);
  EXPECT_EQ(resolved.services[0].cache.maxCachedGroupsPerTrack, 3u);
}
...
TEST(ResolveConfig, CacheCustomValues) {
  ...
  ASSERT_TRUE(result.hasValue());
  const auto& resolved = result.value().config;
  EXPECT_EQ(resolved.services[0].cache.maxCachedTracks, 200u);
  EXPECT_EQ(resolved.services[0].cache.maxCachedGroupsPerTrack, 5u);
}
...
TEST(ResolveConfig, CacheInheritanceFromServiceDefaults) {
  ...
  const auto& resolved = result.value().config;
  ASSERT_EQ(resolved.services.size(), 1);
  EXPECT_EQ(resolved.services[0].cache.maxCachedTracks, 50u);
  EXPECT_EQ(resolved.services[0].cache.maxCachedGroupsPerTrack, 2u);
}
...
TEST(ResolveConfig, CachePerServiceOverridesDefaults) {
  ...
  ASSERT_TRUE(result.hasValue());
  const auto& resolved = result.value().config;
  EXPECT_EQ(resolved.services[0].cache.maxCachedTracks, 300u);
  EXPECT_EQ(resolved.services[0].cache.maxCachedGroupsPerTrack, 8u);
}
...
TEST(ResolveConfig, CachePartialOverrideMergesWithDefaults) {
  ...
  const auto& resolved = result.value().config;
  ASSERT_EQ(resolved.services.size(), 1);
  EXPECT_EQ(resolved.services[0].cache.maxCachedTracks, 500u);
  EXPECT_EQ(resolved.services[0].cache.maxCachedGroupsPerTrack, 2u);
}
...
TEST(ResolveConfig, ResolveExactAuthority) {
  ...
  auto result = resolveConfig(cfg);
  ASSERT_TRUE(result.hasValue());
  const auto& entries = result.value().config.services[0].match;
...
TEST(ResolveConfig, ResolveAnyAuthority) {
  ...
  auto result = resolveConfig(cfg);
  ASSERT_TRUE(result.hasValue());
  const auto& match = result.value().config.services[0].match;
