...
public:
  ...
  // Caps how many AUTHORIZATION_TOKEN params one message may carry; a message
  // over the cap is rejected outright, not truncated. 0 would reject every
  // message carrying a token. Config validation requires >=1 when enabled().
...
// else every successfully-verified token's grants, gated on whether any
// one of them permits Action::ClientSetup.
