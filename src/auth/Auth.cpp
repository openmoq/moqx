...
folly::Expected<folly::Unit, AuthError> authorize(
  ...
  const bool permitted = trackName ? (allowsAny(requestGrants, action, FullTrackName{ns, std::string(*trackName)}) ||
                                       allowsAny(sessionGrants, action, FullTrackName{ns, std::string(*trackName)}))
                                    : (allowsAny(requestGrants, action, ns) ||
                                       allowsAny(sessionGrants, action, ns));
  if (!permitted) {
