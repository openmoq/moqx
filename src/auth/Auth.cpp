...
// catapult's CBOR decoder (parse_bin_match, cwt.cpp) already falls back to
// EXACT for a BinaryMatchType it doesn't recognize, so this switch is
// provably exhaustive today. The XLOG is a tripwire against that decoder
// changing under us, not against this switch drifting -- -Wswitch (made a
// hard error below) already catches a missed BinaryMatchType at compile time.
...
authenticateSetup(const AuthTokenVerifier& verifier, const Parameters& setupParams) {
  ...
  if (!allowsAny(verifiedGrants, Action::ClientSetup, TrackNamespace{})) {
    // At least one token verified but none granted ClientSetup: that's a
    // scope problem (Forbidden), not the crypto/format problem `firstError`
    // holds for the *other* tokens that failed to verify.
    const auto error =
        verifiedGrants.empty() ? firstError.value_or(AuthError::Forbidden) : AuthError::Forbidden;
...
// Non-allocating "is there a matching AUTHORIZATION_TOKEN" check -- avoids
// findAuthTokens()'s allocating vector when only presence, not the tokens
// themselves, is needed.
bool hasAuthToken(const Parameters& params, uint64_t tokenType) {
...
folly::Expected<folly::Unit, AuthError> authorize(
  ...
  } else if (hasAuthToken(params, verifier.tokenType())) {
  ...
  const bool permitted =
      trackName
          ? (allowsAny(requestGrants, action, FullTrackName{ns, std::string(*trackName)}, now) ||
             allowsAny(sessionGrants, action, FullTrackName{ns, std::string(*trackName)}, now) ||
             allows(anonymousGrants, action, FullTrackName{ns, std::string(*trackName)}, now))
          : (allowsAny(requestGrants, action, ns, now) ||
             allowsAny(sessionGrants, action, ns, now) || allows(anonymousGrants, action, ns, now));
  if (!permitted) {
    XLOG(DBG1) << "authorize: action=" << static_cast<uint64_t>(action)
               << " not permitted for ns=" << ns;
    // At least one request token verified but nothing covered the action:
    // that's a scope problem (Forbidden), not the crypto/format problem
    // `firstError` holds for the *other* request tokens that failed to verify.
    const auto error =
        requestGrants.empty() ? firstError.value_or(AuthError::Forbidden) : AuthError::Forbidden;
    return folly::makeUnexpected(error);
  }
  return folly::unit;
}
