...
void ServiceMatcher::PathRuleSet::addRule(
  ...
  if (std::holds_alternative<config::ServiceConfig::MatchEntry::ExactPath>(path)) {
    ...
    exactPaths.try_emplace(val, serviceIndex);
  } else {
    const auto& val = std::get<config::ServiceConfig::MatchEntry::PrefixPath>(path).value;
    prefixPaths.push_back({val, serviceIndex});
  }
}
...
std::optional<size_t> ServiceMatcher::PathRuleSet::match(std::string_view path) const {
...
ServiceMatcher::ServiceMatcher(const std::vector<config::ServiceConfig>& services) {
      ...
      std::visit(
          [&](const auto& auth) {
            ...
            if constexpr (std::is_same_v<A, config::ServiceConfig::MatchEntry::ExactAuthority>) {
              exactAuthorityRules_[auth.value].addRule(entry.path, i);
            } else if constexpr (std::is_same_v<
                                     A,
                                     config::ServiceConfig::MatchEntry::WildcardAuthority>) {
              // "*.x.com" → ".x.com"
              auto suffix = auth.pattern.substr(1);
              wildcardAuthorityRules_[suffix].addRule(entry.path, i);
            } else {
              // AnyAuthority
              anyAuthorityRules_.addRule(entry.path, i);
            }
          },
          entry.authority
      );
