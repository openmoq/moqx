...
AuthConfig::AnonymousScope resolveAnonymousScope(const ParsedAuthConfig::AnonymousScope& scope) {
  ...
  // Unknown action names are skipped defensively; validateAuth() is the real
  // gate and rejects them before resolution ever reaches here.
  for (const auto& name : scope.actions.value()) {
    if (auto action = auth::canonicalAction(name)) {
