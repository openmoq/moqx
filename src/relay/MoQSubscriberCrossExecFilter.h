...
// publish() returns a MoQCrossExecFilter with null inner immediately; a
// fire-and-forget task on targetExec_ calls inner_->publish(), wires the
// real consumer via setInner(), then drives the reply. FIFO ordering
// guarantees setInner() runs before any data writes enqueued afterward.
// goaway() is fire-and-forget.
//
// Requires targetExec_ to be a FIFO executor if call ordering matters.
class MoQSubscriberCrossExecFilter final : public moxygen::Subscriber {
public:
  MoQSubscriberCrossExecFilter(
