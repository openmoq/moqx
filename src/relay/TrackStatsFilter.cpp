...
TrackStatsFilter::~TrackStatsFilter() {
  auto release = [egress = direction_ ==
                           Direction::Egress](const std::shared_ptr<stats::TrackStats>& stats) {
    if (egress) {
  ...
  // Must read the executor before moving stats_ out. The ref drop hops too:
  // ~TrackStats can only evict its collector entry from the owning thread.
  auto* exec = stats_->owningExec();
  if (!exec) {
    return;
  }
  exec->add([stats = std::move(stats_), release] { release(stats); });
}
...
folly::Expected<std::shared_ptr<SubgroupConsumer>, MoQPublishError> TrackStatsFilter::beginSubgroup(
  ...
  countSubgroup();
  return std::static_pointer_cast<SubgroupConsumer>(std::make_shared<TrackStatsSubgroupFilter>(
...
void TrackStatsFilter::countSubgroup() {
  ++stats_->counters().forDirection(direction_).subgroups;
}

void TrackStatsFilter::countObject(uint64_t bytes) {
  auto& counters = stats_->counters();
  ++counters.forDirection(direction_).objects;
  // Sampled on the object header only: payload continuations reach countBytes
  // directly, and re-stamping per chunk would not be a last-*object* time.
...
void TrackStatsFilter::countDatagram(uint64_t bytes) {
  ++stats_->counters().forDirection(direction_).datagrams;
  countObject(bytes);
}

void TrackStatsFilter::countBytes(uint64_t bytes) {
  stats_->counters().forDirection(direction_).bytes += bytes;
}
...
std::shared_ptr<moxygen::TrackConsumer> wrapWithTrackStats(
  ...
  return std::static_pointer_cast<moxygen::TrackConsumer>(std::make_shared<TrackStatsFilter>(
      collector->getOrCreate(ftn),
      direction,
      std::move(downstream)
  ));
}
