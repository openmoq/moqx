...
// downstream of MoQCrossExecForwarderCallback, which handles cross-exec
// dispatch and forwarder pointer recovery.
class WeakRelayForwarderCallback final : public moxygen::MoQForwarder::Callback {
public:
  explicit WeakRelayForwarderCallback(
      std::weak_ptr<moxygen::MoQForwarder::Callback> relay)
