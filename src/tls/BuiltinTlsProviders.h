...
TlsProviderFactory
makeInsecureFactory(std::function<std::shared_ptr<TlsCertProvider>()> creator = nullptr);
...
TlsProviderFactory makeFileFactory(
    std::function<std::shared_ptr<TlsCertProvider>(std::string, std::string)> creator = nullptr
);
