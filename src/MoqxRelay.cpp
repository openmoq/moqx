...
MoqxRelay::PublishSetupResult MoqxRelay::publishWithSession(
  ...
  if (mode() != Mode::LocalForwarder) {
    ...
    chainForwarder = publisherRef.strongOnOwner();
  }
...
bool MoqxRelay::addSubscriberAndPublish(
  ...
  auto forwarder = publisherRef.strongOnOwner();
...
//           ChannelForwarderCallback(publisherRef, subscriberExec)))
//
...
public:
  ChannelForwarderCallback(
      openmoq::moqx::ForwarderRef publisherRef,
      folly::Executor* subscriberExec
  )
      : publisherRef_(std::move(publisherRef)), subscriberExec_(subscriberExec) {}
  ...
  void onEmpty(const moxygen::FullTrackName& /*ftn*/) override {
    if (auto publisher = publisherRef_.pinOnOwner()) {
  ...
  void forwardChanged(const moxygen::FullTrackName&, bool forward) override {
    ...
    launchUpdate(publisherRef_.ownerExec(), doSubscribeUpdate(handle_, forward));
  }
  ...
  void newGroupRequested(const moxygen::FullTrackName&, uint64_t group) override {
    ...
    launchUpdate(publisherRef_.ownerExec(), doNewGroupRequestUpdate(handle_, group));
  }
...
private:
  openmoq::moqx::ForwarderRef publisherRef_;
...
LocalToPublisherCallbacks buildLocalToPublisherCallbacks(
  ...
  auto* publisherExec = publisherRef.ownerExec();
  auto channelCb = std::make_shared<ChannelForwarderCallback>(publisherRef, subscriberExec);
...
folly::coro::Task<void> MoqxRelay::addSubscriberAndPublishViaLocalForwarder(
  ...
  auto finalCallback = co_await publisher->ref.co_with([&](MoQForwarder& publisherFwd) {
    auto cbs = buildLocalToPublisherCallbacks(localReg, ftn, publisher->ref, subscriberExec);
...
folly::coro::Task<MoqxRelay::PublisherAttachment> MoqxRelay::attachNewLocalForwarderOnRelayExec(
  ...
  co_await folly::coro::co_withExecutor(
      ...
      [&]() -> folly::coro::Task<void> {
        ...
        auto cbs =
            buildLocalToPublisherCallbacks(localReg, ftn, attach.publisherRef, subscriberExec);
...
MoqxRelay::subscribeImpl(SubscribeRequest subReq, std::shared_ptr<TrackConsumer> consumer) {
  ...
  if (auto* first = std::get_if<SubscriptionRegistry::FirstSubscriber>(&firstOrSubsequent)) {
    ...
    auto forwarder = sub.forwarder.strongOnOwner();
...
MoqxRelay::fetchImpl(Fetch fetch, std::shared_ptr<FetchConsumer> consumer) {
  ...
  if (joining && mode() != Mode::LocalForwarder) {
    ...
    if (!fetchView) {
      ...
      auto pin = fetchView->forwarder.pinIfOwned();
      XCHECK(pin) << "joining fetch against a remote-owned entry";
      auto res = (*pin)->resolveJoiningFetch(session, *joining);
      if (res.hasError()) {
...
folly::coro::Task<Publisher::TrackStatusResult> MoqxRelay::trackStatusImpl(TrackStatus trackStatus
  ...
  if (mode() == Mode::LocalForwarder) {
    ...
    auto pin = upstreamView->forwarder.pinIfOwned();
    if (pin && (*pin)->numForwardingSubscribers() > 0) {
      trackStatusOk = buildTrackStatusOk(**pin, (bool)upstreamView->handle, trackStatus);
    }
  }
  if (trackStatusOk) {
        ...
        [&](const TrackNamespace& prefix, std::shared_ptr<NamespaceTree::NamespaceNode> current) {
          ...
          current->forEachPublish([&](const std::string& trackName,
            ...
            if (topNView) {
              ...
              if (auto pin = topNView->forwarder.pinIfOwned()) {
                initialPropertyValue = (*pin)->extensions().getIntExtension(propertyType);
              }
...
void MoqxRelay::onTrackEvicted(const FullTrackName& ftn, std::shared_ptr<MoQSession> session) {
  ...
  if (mode() == Mode::LocalForwarder) {
    ...
    auto ref = registry_.getForwarderRef(ftn);
    evict(ref ? ref.strongOnOwner() : nullptr);
  }
}
...
void MoqxRelay::dumpState(RelayStateVisitor& visitor) const {
  ...
  registry_.forEach([&](const SubscriptionRegistry::EntryView& e) {
    ...
    if (auto pin = e.forwarder.pinIfOwned()) {
      info.subscribers = (*pin)->subscriberCount();
      info.forwardingSubscribers = (*pin)->numForwardingSubscribers();
      info.largest = (*pin)->largest();
      info.totalGroupsReceived = (*pin)->totalGroupsReceived();
      info.totalObjectsReceived = (*pin)->totalObjectsReceived();
    }
