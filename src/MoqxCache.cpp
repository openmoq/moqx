...
std::vector<std::pair<AbsoluteLocation, AbsoluteLocation>> getFetchIntervals(
  ...
  if (missLocation.group == hitLocation.group) {
    ...
    if (!isTail) {
      ...
      intervals.emplace_back(missLocation, missLocation.nextGroup().value_or(end));
    }
    return intervals;
  }
  ...
  if (numIntermediateGroups > 0) {
    if (missLocation.object == 0) {
      ...
      intervals.back().first =
          std::max(start, AbsoluteLocation{missLocation.group - numIntermediateGroups, 0});
    } else {
...
bool exists(ObjectStatus status) {
  ...
  return status == ObjectStatus::NORMAL || status == ObjectStatus::END_OF_GROUP ||
...
std::vector<std::pair<AbsoluteLocation, AbsoluteLocation>> getGapRanges(
  ...
  bool isEmpty =
      (order != GroupOrder::NewestFirst)
          ? (start >= end)
          : (start.group < end.group || (start.group == end.group && start.object >= end.object));
  if (isEmpty) {
    return ranges;
  }
  ...
  if (order != GroupOrder::NewestFirst) {
    ...
    XCHECK(gapEnd) << "ascending different-group with end={0,0} should be impossible";
...
bool isGroupNonExistent(const LocationIntervalSet& gaps, uint64_t groupID) {
  ...
  return end &&
         (end->group > groupID || (end->group == groupID && end->object == kLocationMax.object));
}
...
folly::Expected<folly::Unit, MoQPublishError> publishObject(
  ...
  case ObjectStatus::END_OF_TRACK:
    return consumer->endOfTrackAndGroup(current.group, object.subgroup, current.object);
  }
  return folly::makeUnexpected(MoQPublishError{MoQPublishError::API_ERROR, "Unknown status"});
}
...
folly::Expected<folly::Unit, MoQPublishError> MoqxCache::CacheGroup::cacheObject(
  ...
  if (track.gaps.contains({groupID, objectID})) {
    ...
    return folly::makeUnexpected(
        MoQPublishError(MoQPublishError::MALFORMED_TRACK, "Object in known gap")
    );
  }
  ...
  if (status == ObjectStatus::END_OF_GROUP || status == ObjectStatus::END_OF_TRACK) {
  ...
  if (it != objects.end()) {
    ...
    if (status != cachedObject->status) {
      XLOG(ERR) << "Invalid cache status change; objID=" << objectID
                << " status=" << (uint32_t)status << " already exists with different status";
      return folly::makeUnexpected(
    ...
    if (status == ObjectStatus::NORMAL && cachedObject->complete &&
      ...
      return folly::makeUnexpected(
          MoQPublishError(MoQPublishError::MALFORMED_TRACK, "payload mismatch")
      );
    }
    if (cachedObject->forwardingPreferenceIsDatagram != forwardingPreferenceIsDatagram) {
      XLOG(ERR) << "forwardingPreferenceIsDatagram mismatch; objID=" << objectID;
      return folly::makeUnexpected(
...
class MoqxCache::FetchHandle : public Publisher::FetchHandle {
public:
...
MoqxCache::CacheTrack::updateLargest(AbsoluteLocation current, bool eot) {
...
MoqxCache::CacheGroup& MoqxCache::CacheTrack::getOrCreateGroupWithEviction(
    uint64_t groupID,
    MoqxCache& cache,
...
folly::Expected<folly::Unit, MoQPublishError> MoqxCache::CacheTrack::processGapExtensions(
    uint64_t groupID,
    uint64_t objectID,
    const Extensions& objectExtensions
) {
  // Process Prior Group ID Gap extension (0x3C)
  auto priorGroupGap = objectExtensions.getIntExtension(kPriorGroupIdGapExtensionType);
  if (priorGroupGap) {
    uint64_t gap = *priorGroupGap;

    // Validate: gap must not be larger than groupID
    if (gap > groupID) {
      XLOG(ERR) << "Prior Group ID Gap " << gap << " is larger than Group ID " << groupID;
    ...
    if (currentGroup.seenPriorGroupIdGap) {
      if (*currentGroup.seenPriorGroupIdGap != gap) {
        XLOG(ERR) << "Prior Group ID Gap mismatch in group " << groupID << ": previously saw "
                  << *currentGroup.seenPriorGroupIdGap << ", now got " << gap;
        return folly::makeUnexpected(MoQPublishError(
            MoQPublishError::MALFORMED_TRACK,
      ...
      if (gap > 0) {
        ...
        for (uint64_t g = groupID - checkCount; g < groupID; ++g) {
          ...
          if (groupIt != groups.end() && !groupIt->second->objects.empty()) {
            XLOG(ERR) << "Prior Group ID Gap covers existing object in group " << g;
            return folly::makeUnexpected(MoQPublishError(
                MoQPublishError::MALFORMED_TRACK,
  ...
  auto priorObjectGap = objectExtensions.getIntExtension(kPriorObjectIdGapExtensionType);
  if (priorObjectGap) {
    uint64_t gap = *priorObjectGap;

    // Validate: gap must not be larger than objectID
    if (gap > objectID) {
      XLOG(ERR) << "Prior Object ID Gap " << gap << " is larger than Object ID " << objectID;
      return folly::makeUnexpected(MoQPublishError(
          MoQPublishError::MALFORMED_TRACK,
    ...
    if (gap > 0) {
      ...
      for (uint64_t o = objectID - checkCount; o < objectID; ++o) {
        ...
        if (it != group.objects.end()) {
          XLOG(ERR) << "Prior Object ID Gap covers existing object " << o << " in group "
                    << groupID;
          return folly::makeUnexpected(MoQPublishError(
              MoQPublishError::MALFORMED_TRACK,
...
public:
  SubgroupWriteback(
      ...
      : group_(group), subgroup_(subgroup), consumer_(std::move(consumer)),
  ...
  void checkpoint() override { return consumer_->checkpoint(); }
  ...
  void reset(ResetStreamErrorCode error) override { return consumer_->reset(error); }
...
class MoqxCache::SubscribeWriteback : public TrackConsumer {
public:
  SubscribeWriteback(
      ...
      : consumer_(std::move(consumer)), trackPtr_(std::move(trackPtr)), track_(*trackPtr_),
...
MoqxCache::FetchWriteback* MoqxCache::CacheTrack::findFetchInProgress(AbsoluteLocation loc) {
...
public:
  ...
  folly::coro::Task<void> waitFor(AbsoluteLocation loc) {
    ...
    while (!token.isCancellationRequested() && fetchRangeIt_.isValid() && loc >= *fetchRangeIt_) {
  ...
  void checkpoint() override { consumer_->checkpoint(); }
  ...
  folly::Expected<folly::Unit, MoQPublishError> beginObject(
    ...
    auto res = cacheImpl(gID, sgID, objID, kNormal, ext, std::move(payload), false, false);
    if (!res) {
      return res;
    }
    currentLength_ = len;
  ...
  endOfGroup(uint64_t gID, uint64_t sgID, uint64_t objID, bool fin) override {
    ...
    auto res = cacheImpl(gID, sgID, objID, kEndOfGroup, noExtensions(), nullptr, true, fin);
    if (!res) {
      return res;
    }
    return consumer_->endOfGroup(gID, sgID, objID, fin && proxyFin_);
  ...
  endOfTrackAndGroup(uint64_t gID, uint64_t sgID, uint64_t objID) override {
    ...
    auto res = cacheImpl(gID, sgID, objID, kEndOfTrack, noExtensions(), nullptr, true, true);
    if (!res) {
      return res;
    }
    return consumer_->endOfTrackAndGroup(gID, sgID, objID);
...
private:
  ...
  folly::Expected<folly::Unit, MoQPublishError> cacheImpl(
    ...
    auto& group = fetchRangeIt_.track->getOrCreateGroupWithEviction(groupID, cache_, ftn_);
    auto cacheRes = cache_.cacheObjectAndUpdateBytes(
        group,
        *fetchRangeIt_.track,
        groupID,
    ...
    auto res = fetchRangeIt_.track->updateLargest({groupID, objectID}, isEndOfTrack(status));
    if (!res) {
      updateInProgress();
      return res;
    }
...
std::shared_ptr<TrackConsumer> MoqxCache::getSubscribeWriteback(
  ...
  return std::make_shared<SubscribeWriteback>(std::move(consumer), trackIt->second, *this, ftn);
}
...
folly::coro::Task<Publisher::FetchResult> MoqxCache::fetchImpl(
  ...
  while (!token.isCancellationRequested() && (*fetchRangeIt) != fetchRangeIt.end()) {
    ...
    if (blockingWriteback) {
      XLOG(DBG1) << "fetchInProgress for {" << current.group << "," << current.object << "}";
    ...
    // stays authoritative.
    if (fetchStart) {
      XLOG(DBG1) << "flushing accumulated upstream miss at boundary {" << current.group << ","
                 << current.object << "} (" << (cachedObject ? "hit" : "gap") << ")";
    ...
    auto res = publishObject(object->status, consumer, current, *object, lastObject);
    if (res.hasError()) {
      if (res.error().code == MoQPublishError::BLOCKED) {
        ...
        co_return folly::makeUnexpected(FetchError{
            ...
            folly::to<std::string>("Consumer error on object=", res.error().msg)
        });
      }
...
MoqxCache::getCachedObjectMaybe(CacheTrack& track, AbsoluteLocation current, TimePoint now) {
  ...
  if (objIt == group->objects.end()) {
    XLOG(DBG1) << "object cache miss for {" << current.group << "," << current.object << "}";
    return nullptr;
  }

  auto effectiveDuration =
      track.maxCacheDuration ? track.maxCacheDuration : defaultMaxCacheDuration_;
  if (effectiveDuration) {
    auto age = now - objIt->second->cachedAt;
    if (age > *effectiveDuration) {
      XLOG(DBG1) << "object expired for {" << current.group << "," << current.object << "}";
  ...
  if (!objIt->second->complete) {
    XLOG(DBG1) << "object incomplete for {" << current.group << "," << current.object << "}";
    return nullptr;
  }
...
folly::coro::Task<Publisher::FetchResult> MoqxCache::fetchUpstream(
...
MoqxCache::handleBlocked(std::shared_ptr<FetchConsumer> consumer, const Fetch& fetch) {
  ...
  if (!awaitRes) {
    ...
    co_return folly::makeUnexpected(FetchError{
        fetch.requestID,
        FetchErrorCode::INTERNAL_ERROR,
        folly::to<std::string>("Consumer error awaiting ready=", awaitRes.error().msg)
    });
  }
...
void MoqxCache::setMaxCacheDuration(const FullTrackName& ftn, std::chrono::milliseconds duration) {
...
void MoqxCache::clearMaxCacheDuration(const FullTrackName& ftn) {
  auto it = cache_.find(ftn);
  if (it != cache_.end()) {
...
MoqxCache::FetchRangeIterator::FetchRangeIterator(
    AbsoluteLocation start,
    AbsoluteLocation end,
    GroupOrder order,
    std::shared_ptr<CacheTrack> track
...
bool MoqxCache::FetchRangeIterator::isValid() const {
  return isValid_;
}

void MoqxCache::FetchRangeIterator::invalidate() {
  current_ = maxLocation;
  isValid_ = false;
}

void MoqxCache::FetchRangeIterator::advanceTo(const AbsoluteLocation& loc) {
  current_ = loc;
...
const AbsoluteLocation& MoqxCache::FetchRangeIterator::operator*() const {
  return current_;
}
...
void MoqxCache::FetchRangeIterator::next() {
  if (!isValid_) {
    return;
  }
  advanceOne();
...
void MoqxCache::FetchRangeIterator::advanceOne() {
  if (!isValid_) {
    return;
  }
  if (current_ == end_) {
  ...
  auto groupEndMaybe = findGroupEndMaybe(current_.group, cachedGroupId_, cachedGroupPtr_);
  XLOG(DBG2) << "advanceOne: current={" << current_.group << "," << current_.object
             << "} groupEndMaybe=" << (groupEndMaybe ? std::to_string(*groupEndMaybe) : "nullopt");
  ...
  XLOG(DBG2) << "advanceOne: new current={" << current_.group << "," << current_.object << "}";
}

void MoqxCache::FetchRangeIterator::skipGaps() {
  while (isValid_ && track->gaps.contains(current_)) {
    XLOG(DBG2) << "skipGaps: current={" << current_.group << "," << current_.object
               << "} is in gap";
    ...
    if (afterGap) {
      XLOG(DBG2) << "skipGaps: afterGap={" << afterGap->group << "," << afterGap->object << "}";
      ...
      bool inRange =
          (order != GroupOrder::NewestFirst) ? (*afterGap < end_) : (*afterGap >= minLocation);
      if (inRange) {
        current_ = *afterGap;
        XLOG(DBG2) << "skipGaps: moved to {" << current_.group << "," << current_.object << "}";
      } else {
        // Gap extends to or past end - clamp to end
        XLOG(DBG2) << "skipGaps: clamping to end {" << end_.group << "," << end_.object << "}";
        current_ = end_;
...
void MoqxCache::FetchRangeIterator::skipUncached() {
...
std::optional<AbsoluteLocation> MoqxCache::FetchRangeIterator::findPrevGroupCachedPosition() const {
...
*/
std::optional<uint64_t> MoqxCache::FetchRangeIterator::findGroupEndMaybe(
    uint64_t currGroup,
    uint64_t& cachedGroupId,
    std::shared_ptr<CacheGroup>& cachedGroupPtr
) const {
  ...
  if (cachedGroupId != currGroup) {
    ...
    cachedGroupPtr = (groupIt != track->groups.end()) ? groupIt->second : nullptr;
  ...
  const uint64_t endIncludingMaxCached =
      (maxObjectInGroup < kLocationMax.object) ? maxObjectInGroup + 1 : maxObjectInGroup;
  const uint64_t firstUncachedObject = (endIncludingMaxCached < kLocationMax.object)
...
AbsoluteLocation MoqxCache::FetchRangeIterator::end() {
  ...
  // desc
  auto endGroupLastObjMaybe = findGroupEndMaybe(end_.group, cachedEndGroupId_, cachedEndGroupPtr_);
...
void MoqxCache::addTrackToLRU(const FullTrackName& ftn, CacheTrack& track) {
...
void MoqxCache::removeTrackFromLRU(CacheTrack& track) {
  if (!track.lruIter_.hasValue()) {
    // Not in LRU
    return;
  }
...
void MoqxCache::onTrackBecameEvictable(const FullTrackName& ftn) {
  auto it = cache_.find(ftn);
  if (it == cache_.end()) {
...
void MoqxCache::removeGroupFromLRU(CacheGroup& group, CacheTrack& track) {
  if (!group.lruIter_.hasValue()) {
    // Not in LRU
    return;
  }
...
bool MoqxCache::evictOldestTrackIfNeeded() {
  ...
  if (trackLRU_.empty()) {
    ...
    XLOG(DBG1) << "Cannot evict any track, all have active operations. "
               << "Cache size: " << cache_.size() << ", limit: " << maxCachedTracks_;
    return false;
  }

  // Take oldest evictable track from back of LRU
  const FullTrackName& oldestTrack = trackLRU_.back();
  XLOG(DBG1) << "Evicting oldest track: " << oldestTrack << " (cache size: " << cache_.size()
             << ")";
  evictTrack(oldestTrack);
  return true;
}

size_t MoqxCache::evictTrack(const FullTrackName& ftn) {
  auto it = cache_.find(ftn);
  if (it == cache_.end()) {
    return 0;
  }
...
size_t MoqxCache::purge(const TrackNamespace& ns) {
...
void MoqxCache::evictOldestGroupsIfNeeded(CacheTrack& track) {
  ...
  if (track.groups.size() > maxCachedGroupsPerTrack_ && track.groupLRU.empty()) {
...
void MoqxCache::evictGroup(CacheTrack& track, uint64_t groupID) {
...
bool MoqxCache::evictForByteLimitIfNeeded() {
...
folly::Expected<folly::Unit, MoQPublishError> MoqxCache::cacheObjectAndUpdateBytes(
    CacheGroup& group,
    CacheTrack& track,
    uint64_t groupID,
    uint64_t subgroup,
