...
struct TrackCounters {
  ...
  uint64_t subgroupsReceived{0};
  uint64_t objectsReceived{0};
  uint64_t datagramsReceived{0};
  uint64_t bytesReceived{0};

  uint64_t groupsSent{0};
  uint64_t subgroupsSent{0};
  uint64_t objectsSent{0};
  ...
  TrackCounters& operator+=(const TrackCounters& o) {
    groupsReceived += o.groupsReceived;
    subgroupsReceived += o.subgroupsReceived;
    objectsReceived += o.objectsReceived;
    datagramsReceived += o.datagramsReceived;
    bytesReceived += o.bytesReceived;

    groupsSent += o.groupsSent;
    subgroupsSent += o.subgroupsSent;
    objectsSent += o.objectsSent;
    datagramsSent += o.datagramsSent;
...
public:
  ...
  std::shared_ptr<TrackStats> create(const moxygen::FullTrackName& ftn) {
    ...
    stats_[ftn] = Slot{stats.get(), stats};
    return stats;
  }
  ...
  std::shared_ptr<TrackStats> get(const moxygen::FullTrackName& ftn) const {
    ...
    // A slot can outlive its TrackStats when teardown ran off-thread and could
    // not reach us; treat the expired weak ref as absent.
  ...
  void forEach(folly::FunctionRef<void(const TrackStats&)> fn) const {
    checkThread();
    for (const auto& [ftn, slot] : stats_) {
      if (auto stats = slot.weak.lock()) {
        fn(*stats);
      }
    }
  }
...
private:
  ...
  folly::F14FastMap<moxygen::FullTrackName, Slot, moxygen::FullTrackName::hash> stats_;
};
...
inline TrackStats::TrackStats(std::weak_ptr<TrackStatsCollector> collector, moxygen::FullTrackName ftn)
...
// reach the owning thread — the slot's weak ref then reports the entry absent.
