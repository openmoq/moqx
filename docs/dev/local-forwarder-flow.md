...
identity, with no dereference or strong access — so `relayExec_` can neither read the forwarder
nor run `~MoQForwarder`. Relay-side decisions about a forwarder travel to its executor as
messages (`ForwarderRef::post`), never as pointers.
