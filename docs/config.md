...
| `require_setup_token` | `true` | Requires a valid setup token authorizing `client_setup` during CLIENT_SETUP. If `false`, clients can connect without setup grants; per-request actions still need an authorized token or a matching `anonymous_claim` entry. |
...
| `max_tokens_per_message` | none | Required when `enabled: true`, directly or via `service_defaults.auth.max_tokens_per_message`. Caps how many `AUTHORIZATION_TOKEN` params a single CLIENT_SETUP or request message may carry; a message over the cap is rejected outright, not truncated. Bounds per-message verification cost against a client attaching many tokens to one message. Must be >= 1. |
...
A message (CLIENT_SETUP or any per-request message) may carry more than one
`AUTHORIZATION_TOKEN`, up to `max_tokens_per_message`; a message carrying more
than that is rejected outright. Within the cap, a request is authorized if
