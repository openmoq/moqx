...
# hard error pointing at the superbuild. See BUILD.md#how-dependencies-work.
#
# Included by CMakeLists.txt, after cmake/dependencies.cmake.
#
# Knobs:
#   MOQX_PLATFORM          override the auto-detected platform tag
#   MOXYGEN_RELEASE_TAG    release tag to fetch from (declared in
...
function(_moqx_detect_platform OUT_VAR)
  ...
  # CMAKE_HOST_SYSTEM_PROCESSOR comes from project()/enable_language(), so it is
  # empty under `cmake -P`. uname -m is CMake's own probe, so script mode and
  # project mode agree; disagreeing seeds a directory the build never looks at.
  ...
  # CMAKE_HOST_APPLE, not APPLE: this picks the tarball for the machine doing the
  # download, and APPLE describes the target and is unset in script mode.
  if(CMAKE_HOST_APPLE)
...
if(EXISTS "${_install_dir}/lib/cmake/moxygen/moxygen-config.cmake"
  ...
  # Honors MOXYGEN_RELEASE_TAG; the release read below verifies the commit
  # against the pin either way, so a wrong tag fails before any download.
  ...
  if(NOT _rel_commit STREQUAL _pin_lc)
    message(FATAL_ERROR
      ...
      "(scripts/configure.sh --moxygen from-source).")
  endif()
  ...
  # Unchecked, the error text lands in _asset_count and the loop below is skipped,
  # reporting a malformed response as a release that publishes nothing.
  if(NOT _json_err STREQUAL "NOTFOUND")
    message(FATAL_ERROR
      "moxygen: unexpected release JSON for '${_rel_tag}' — no asset list (${_json_err}).")
  endif()
  ...
  # The extracted tree is the artifact; keeping the archive beside it doubles
  # what a cache of this root has to carry.
  file(REMOVE "${_dl}")
  message(STATUS
